...
void PrintTabularResult(vector<TableData*>& table_data)
{
    ...
    {
        FormatDataInTabularForm(entry->root->letter);
        FormatDataInTabularForm(entry->root->frequency);
        FormatDataInTabularForm(entry->code_array);
        FormatDataInTabularForm(entry->code_array.length());
        FormatDataInTabularForm(entry->code_array.length() * entry->root->frequency);
        cout << endl;
    }
}
...
int main()
{
    ...
    sort(table_data.begin(), table_data.end(), [](TableData* a, TableData* b) {
    ...
    for (const auto& entry : table_data)
    {
        sum_of_letters_frequencyxlenght += (entry->code_array.length() * entry->root->frequency);
    }
